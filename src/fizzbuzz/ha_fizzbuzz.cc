...
/**
  ...
  @brief
  The ha_fizzbuzz engine is a stubbed storage engine for fizzbuzz purposes only;
  it does nothing at this point. Its purpose is to provide a source
  code illustration of how to begin writing new storage engines; see also
  /storage/fizzbuzz/ha_fizzbuzz.h.

  @details
  ha_fizzbuzz will let you create/open/delete tables, but
  nothing further (for fizzbuzz, indexes are not supported nor can data
  be stored in the table). Use this fizzbuzz as a template for
  implementing the same functionality in your own storage engine. You
  can enable the fizzbuzz storage engine in your build by doing the
  following during your build process:<br> ./configure
  --with-fizzbuzz-storage-engine

  Once this is done, MySQL will let you create tables with:<br>
  CREATE TABLE \<table name\> (...) ENGINE=FIZZBUZZ;

  The fizzbuzz storage engine is set up to use table locks. It
  implements an fizzbuzz "SHARE" that is inserted into a hash by table
  name. You can use this to store information of state that any
  fizzbuzz handler object will be able to see when it is using that
  table.

  Please read the object definition in ha_fizzbuzz.h before reading the rest
  of this file.

  @note
  When you create an FIZZBUZZ table, the MySQL Server creates a table .frm
  (format) file in the database directory, using the table name as the file
  name as is customary with MySQL. No other files are created. To get an idea
  of what occurs, here is an fizzbuzz select that would do a scan of an entire
  table:

  @code
  ha_fizzbuzz::store_lock
  ha_fizzbuzz::external_lock
  ha_fizzbuzz::info
  ha_fizzbuzz::rnd_init
  ha_fizzbuzz::extra
  ha_fizzbuzz::rnd_next
  ha_fizzbuzz::rnd_next
  ha_fizzbuzz::rnd_next
  ha_fizzbuzz::rnd_next
  ha_fizzbuzz::rnd_next
  ...
  Here you see that the fizzbuzz storage engine has 9 rows called before
...
handlerton *fizzbuzz_hton;

/* Interface to mysqld, to check system tables supported by SE */
static bool fizzbuzz_is_supported_system_table(const char *db,
...
static int fizzbuzz_init_func(void *p) {
  ...
  fizzbuzz_hton = (handlerton *)p;
  fizzbuzz_hton->state = SHOW_OPTION_YES;
  fizzbuzz_hton->create = fizzbuzz_create_handler;
  fizzbuzz_hton->flags = HTON_CAN_RECREATE;
  fizzbuzz_hton->is_supported_system_table = fizzbuzz_is_supported_system_table;
...
static handler *fizzbuzz_create_handler(handlerton *hton, TABLE_SHARE *table,
  ...
  return new (mem_root) ha_fizzbuzz(hton, table);
}

ha_fizzbuzz::ha_fizzbuzz(handlerton *hton, TABLE_SHARE *table_arg)
...
*/
static st_handler_tablename ha_fizzbuzz_system_tables[] = {
    {(const char *)NULL, (const char *)NULL}};
...
static bool fizzbuzz_is_supported_system_table(const char *db,
  ...
  systab = ha_fizzbuzz_system_tables;
...
int ha_fizzbuzz::open(const char *, int, uint, const dd::Table *) {
  DBUG_ENTER("ha_fizzbuzz::open");
...
int ha_fizzbuzz::close(void) {
  DBUG_ENTER("ha_fizzbuzz::close");
  DBUG_RETURN(0);
}
...
/**
  ...
  See ha_tina.cc for an fizzbuzz of extracting all of the data as strings.
  ha_berekly.cc has an fizzbuzz of how to store it intact by "packing" it
...
int ha_fizzbuzz::index_read_map(uchar *, const uchar *, key_part_map,
                               enum ha_rkey_function) {
  int rc;
  DBUG_ENTER("ha_fizzbuzz::index_read");
...
int ha_fizzbuzz::index_next(uchar *) {
  int rc;
  DBUG_ENTER("ha_fizzbuzz::index_next");
  rc = HA_ERR_WRONG_COMMAND;
  DBUG_RETURN(rc);
}
...
int ha_fizzbuzz::index_prev(uchar *) {
  int rc;
  DBUG_ENTER("ha_fizzbuzz::index_prev");
  rc = HA_ERR_WRONG_COMMAND;
  DBUG_RETURN(rc);
}
...
*/
int ha_fizzbuzz::index_first(uchar *) {
  int rc;
  DBUG_ENTER("ha_fizzbuzz::index_first");
  rc = HA_ERR_WRONG_COMMAND;
  DBUG_RETURN(rc);
}
...
/**
  ...
  opt_range.cc, opt_sum.cc, sql_handler.cc and sql_select.cc
*/
int ha_fizzbuzz::index_last(uchar *) {
  int rc;
  DBUG_ENTER("ha_fizzbuzz::index_last");
  rc = HA_ERR_WRONG_COMMAND;
  DBUG_RETURN(rc);
}
...
/**
  ...
  scan. See the fizzbuzz in the introduction at the top of this file to see when
  rnd_init() is called.
...
int ha_fizzbuzz::rnd_end() {
  DBUG_ENTER("ha_fizzbuzz::rnd_end");
  DBUG_RETURN(0);
}
...
*/
void ha_fizzbuzz::position(const uchar *) {
  DBUG_ENTER("ha_fizzbuzz::position");
  DBUG_VOID_RETURN;
}
...
*/
int ha_fizzbuzz::rnd_pos(uchar *, uchar *) {
  int rc;
  DBUG_ENTER("ha_fizzbuzz::rnd_pos");
  rc = HA_ERR_WRONG_COMMAND;
  DBUG_RETURN(rc);
}
...
/**
  ...
  sql_show.cc, sql_table.cc, sql_union.cc and sql_update.cc
*/
int ha_fizzbuzz::info(uint) {
  DBUG_ENTER("ha_fizzbuzz::info");
  DBUG_RETURN(0);
}
...
/**
  ...
  ha_innodb.cc
*/
int ha_fizzbuzz::extra(enum ha_extra_function) {
  DBUG_ENTER("ha_fizzbuzz::extra");
  DBUG_RETURN(0);
}
...
*/
int ha_fizzbuzz::delete_all_rows() {
  DBUG_ENTER("ha_fizzbuzz::delete_all_rows");
...
*/
int ha_fizzbuzz::external_lock(THD *, int) {
  DBUG_ENTER("ha_fizzbuzz::external_lock");
  DBUG_RETURN(0);
}
...
/**
  ...
  Berkeley DB, for fizzbuzz, changes all WRITE locks to TL_WRITE_ALLOW_WRITE
...
*/
THR_LOCK_DATA **ha_fizzbuzz::store_lock(THD *, THR_LOCK_DATA **to,
...
*/
int ha_fizzbuzz::delete_table(const char *, const dd::Table *) {
  DBUG_ENTER("ha_fizzbuzz::delete_table");
...
*/
int ha_fizzbuzz::rename_table(const char *, const char *, const dd::Table *,
                             dd::Table *) {
  DBUG_ENTER("ha_fizzbuzz::rename_table ");
  DBUG_RETURN(HA_ERR_WRONG_COMMAND);
}
...
*/
ha_rows ha_fizzbuzz::records_in_range(uint, key_range *, key_range *) {
  DBUG_ENTER("ha_fizzbuzz::records_in_range");
...
int ha_fizzbuzz::create(const char *name, TABLE *, HA_CREATE_INFO *,
                       dd::Table *) {
  DBUG_ENTER("ha_fizzbuzz::create");
  /*
  ...
  */
  ...
  /*
    It's just an fizzbuzz of THDVAR_SET() usage below.
  */
  THD *thd = ha_thd();
...
struct st_mysql_storage_engine fizzbuzz_storage_engine = {
...
static SYS_VAR *fizzbuzz_system_variables[] = {
...
// this is an fizzbuzz of SHOW_FUNC
static int show_func_fizzbuzz(MYSQL_THD, SHOW_VAR *var, char *buf) {
  var->type = SHOW_CHAR;
...
fizzbuzz_vars_t fizzbuzz_vars = {100, 20.01, "three hundred", true, 0, 8250};

static SHOW_VAR show_status_fizzbuzz[] = {
    {"var1", (char *)&fizzbuzz_vars.var1, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"var2", (char *)&fizzbuzz_vars.var2, SHOW_DOUBLE, SHOW_SCOPE_GLOBAL},
    {0, 0, SHOW_UNDEF, SHOW_SCOPE_UNDEF}  // null terminator required
};

static SHOW_VAR show_array_fizzbuzz[] = {
    {"array", (char *)show_status_fizzbuzz, SHOW_ARRAY, SHOW_SCOPE_GLOBAL},
    {"var3", (char *)&fizzbuzz_vars.var3, SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"var4", (char *)&fizzbuzz_vars.var4, SHOW_BOOL, SHOW_SCOPE_GLOBAL},
    {0, 0, SHOW_UNDEF, SHOW_SCOPE_UNDEF}};

static SHOW_VAR func_status[] = {
    {"fizzbuzz_func_fizzbuzz", (char *)show_func_fizzbuzz, SHOW_FUNC,
     SHOW_SCOPE_GLOBAL},
    {"fizzbuzz_status_var5", (char *)&fizzbuzz_vars.var5, SHOW_BOOL,
     SHOW_SCOPE_GLOBAL},
    {"fizzbuzz_status_var6", (char *)&fizzbuzz_vars.var6, SHOW_LONG,
     SHOW_SCOPE_GLOBAL},
    {"fizzbuzz_status", (char *)show_array_fizzbuzz, SHOW_ARRAY,
     SHOW_SCOPE_GLOBAL},
    {0, 0, SHOW_UNDEF, SHOW_SCOPE_UNDEF}};

mysql_declare_plugin(fizzbuzz){
    ...
    fizzbuzz_system_variables, /* system variables */
    NULL,                     /* config options */
    0,                        /* flags */
